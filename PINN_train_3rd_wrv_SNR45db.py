...
x_train = train_dataset[:,0:5200].astype(float)
Y_train = train_dataset[:,7000].astype(float)
x_test  = val_dataset[:,0:5200].astype(float)
Y_test  = val_dataset[:,7000].astype(float)
...
plt.savefig('Learning Curve - PINN - 45dB.png')
plt.show()

# Prediction Results
plt.figure(figsize=(6,6))
plt.scatter(Y_test, pred, alpha=0.6)
plt.xlabel('Actual Inertia (s)'); plt.ylabel('Predicted Inertia (s)')
plt.grid(True, linestyle='--', alpha=0.5)
plt.savefig('Prediction Results - PINN - 45dB.png')
plt.show()
...
plt.grid(True, linestyle="--", alpha=0.5)
plt.savefig('Distribution of Absolute Errors - PINN - 45dB.png')
plt.show()
...
plt.grid(True, linestyle="--", alpha=0.5)
plt.savefig('Absolute Error per case - PINN - 45dB.png')
plt.show()
...
plt.savefig("Metrics Summary - PINN - 45dB.png", bbox_inches="tight", dpi=300)
plt.show()